...
[RoutePrefix("api/ERP/PartClassPlants")]
public class ERPPartClassPlantsController : ERPBaseController
{
	public IERPPartClassPlantModel erpPartClassPlantModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPPartClassPlantDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPPartClassPlantDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllPartClassPlantAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpPartClassPlantModel = new ERPPartClassPlantModel())
		{
			return await RunApiMethod(base.Request, erpPartClassPlantModel, () => erpPartClassPlantModel.ValidateRequest_GetAllPartClassPlants(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpPartClassPlantModel.Process_GetAllPartClassPlants(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a PartClassPlant based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPPartClassPlantDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPPartClassPlantDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/INVENTORY" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetPartClassPlantAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpPartClassPlantModel = new ERPPartClassPlantModel())
		{
			return await RunApiMethod(base.Request, erpPartClassPlantModel, () => erpPartClassPlantModel.ValidateRequest_GetPartClassPlant(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpPartClassPlantModel.Process_GetPartClassPlant(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
