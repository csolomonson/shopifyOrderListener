...
[RoutePrefix("api/ERP/EmployeeSalesBudgets")]
public class ERPEmployeeSalesBudgetsController : ERPBaseController
{
	public IERPEmployeeSalesBudgetModel erpEmployeeSalesBudgetModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPEmployeeSalesBudgetDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPEmployeeSalesBudgetDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllEmployeeSalesBudgetAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpEmployeeSalesBudgetModel = new ERPEmployeeSalesBudgetModel())
		{
			return await RunApiMethod(base.Request, erpEmployeeSalesBudgetModel, () => erpEmployeeSalesBudgetModel.ValidateRequest_GetAllEmployeeSalesBudgets(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpEmployeeSalesBudgetModel.Process_GetAllEmployeeSalesBudgets(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a EmployeeSalesBudget based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPEmployeeSalesBudgetDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPEmployeeSalesBudgetDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/EMPLOYEE" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetEmployeeSalesBudgetAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpEmployeeSalesBudgetModel = new ERPEmployeeSalesBudgetModel())
		{
			return await RunApiMethod(base.Request, erpEmployeeSalesBudgetModel, () => erpEmployeeSalesBudgetModel.ValidateRequest_GetEmployeeSalesBudget(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpEmployeeSalesBudgetModel.Process_GetEmployeeSalesBudget(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
