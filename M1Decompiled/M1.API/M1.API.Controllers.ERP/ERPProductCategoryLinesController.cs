...
[RoutePrefix("api/ERP/ProductCategoryLines")]
public class ERPProductCategoryLinesController : ERPBaseController
{
	public IERPProductCategoryLineModel erpProductCategoryLineModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPProductCategoryLineDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPProductCategoryLineDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllProductCategoryLineAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpProductCategoryLineModel = new ERPProductCategoryLineModel())
		{
			return await RunApiMethod(base.Request, erpProductCategoryLineModel, () => erpProductCategoryLineModel.ValidateRequest_GetAllProductCategoryLines(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpProductCategoryLineModel.Process_GetAllProductCategoryLines(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ProductCategoryLine based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPProductCategoryLineDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPProductCategoryLineDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/INVENTORY" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetProductCategoryLineAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpProductCategoryLineModel = new ERPProductCategoryLineModel())
		{
			return await RunApiMethod(base.Request, erpProductCategoryLineModel, () => erpProductCategoryLineModel.ValidateRequest_GetProductCategoryLine(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpProductCategoryLineModel.Process_GetProductCategoryLine(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
