...
[RoutePrefix("api/ERP/ToolCategories")]
public class ERPToolCategoriesController : ERPBaseController
{
	public IERPToolCategoryModel erpToolCategoryModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPToolCategoryDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPToolCategoryDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllToolCategoryAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpToolCategoryModel = new ERPToolCategoryModel())
		{
			return await RunApiMethod(base.Request, erpToolCategoryModel, () => erpToolCategoryModel.ValidateRequest_GetAllToolCategories(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpToolCategoryModel.Process_GetAllToolCategories(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ToolCategory based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPToolCategoryDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPToolCategoryDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/PRODUCTION" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetToolCategoryAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpToolCategoryModel = new ERPToolCategoryModel())
		{
			return await RunApiMethod(base.Request, erpToolCategoryModel, () => erpToolCategoryModel.ValidateRequest_GetToolCategory(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpToolCategoryModel.Process_GetToolCategory(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
