...
[RoutePrefix("api/ERP/PartGroupPlants")]
public class ERPPartGroupPlantsController : ERPBaseController
{
	public IERPPartGroupPlantModel erpPartGroupPlantModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPPartGroupPlantDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPPartGroupPlantDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllPartGroupPlantAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpPartGroupPlantModel = new ERPPartGroupPlantModel())
		{
			return await RunApiMethod(base.Request, erpPartGroupPlantModel, () => erpPartGroupPlantModel.ValidateRequest_GetAllPartGroupPlants(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpPartGroupPlantModel.Process_GetAllPartGroupPlants(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a PartGroupPlant based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPPartGroupPlantDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPPartGroupPlantDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/INVENTORY" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetPartGroupPlantAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpPartGroupPlantModel = new ERPPartGroupPlantModel())
		{
			return await RunApiMethod(base.Request, erpPartGroupPlantModel, () => erpPartGroupPlantModel.ValidateRequest_GetPartGroupPlant(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpPartGroupPlantModel.Process_GetPartGroupPlant(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
