...
[RoutePrefix("api/ERP/EmployeeSkills")]
public class ERPEmployeeSkillsController : ERPBaseController
{
	public IERPEmployeeSkillModel erpEmployeeSkillModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPEmployeeSkillDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPEmployeeSkillDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllEmployeeSkillAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpEmployeeSkillModel = new ERPEmployeeSkillModel())
		{
			return await RunApiMethod(base.Request, erpEmployeeSkillModel, () => erpEmployeeSkillModel.ValidateRequest_GetAllEmployeeSkills(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpEmployeeSkillModel.Process_GetAllEmployeeSkills(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a EmployeeSkill based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPEmployeeSkillDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPEmployeeSkillDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/EMPLOYEE" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetEmployeeSkillAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpEmployeeSkillModel = new ERPEmployeeSkillModel())
		{
			return await RunApiMethod(base.Request, erpEmployeeSkillModel, () => erpEmployeeSkillModel.ValidateRequest_GetEmployeeSkill(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpEmployeeSkillModel.Process_GetEmployeeSkill(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
