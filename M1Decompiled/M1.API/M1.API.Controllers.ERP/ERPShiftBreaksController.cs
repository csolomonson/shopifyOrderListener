...
[RoutePrefix("api/ERP/ShiftBreaks")]
public class ERPShiftBreaksController : ERPBaseController
{
	public IERPShiftBreakModel erpShiftBreakModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPShiftBreakDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPShiftBreakDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllShiftBreakAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpShiftBreakModel = new ERPShiftBreakModel())
		{
			return await RunApiMethod(base.Request, erpShiftBreakModel, () => erpShiftBreakModel.ValidateRequest_GetAllShiftBreaks(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpShiftBreakModel.Process_GetAllShiftBreaks(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ShiftBreak based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPShiftBreakDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPShiftBreakDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/EMPLOYEE" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetShiftBreakAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpShiftBreakModel = new ERPShiftBreakModel())
		{
			return await RunApiMethod(base.Request, erpShiftBreakModel, () => erpShiftBreakModel.ValidateRequest_GetShiftBreak(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpShiftBreakModel.Process_GetShiftBreak(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
