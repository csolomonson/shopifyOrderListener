...
[RoutePrefix("api/ERP/ScheduleTaskBuckets")]
public class ERPScheduleTaskBucketsController : ERPBaseController
{
	public IERPScheduleTaskBucketModel erpScheduleTaskBucketModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPScheduleTaskBucketDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPScheduleTaskBucketDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllScheduleTaskBucketAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpScheduleTaskBucketModel = new ERPScheduleTaskBucketModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTaskBucketModel, () => erpScheduleTaskBucketModel.ValidateRequest_GetAllScheduleTaskBuckets(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTaskBucketModel.Process_GetAllScheduleTaskBuckets(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ScheduleTaskBucket based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPScheduleTaskBucketDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPScheduleTaskBucketDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/SCHEDULING" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetScheduleTaskBucketAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpScheduleTaskBucketModel = new ERPScheduleTaskBucketModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTaskBucketModel, () => erpScheduleTaskBucketModel.ValidateRequest_GetScheduleTaskBucket(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTaskBucketModel.Process_GetScheduleTaskBucket(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
