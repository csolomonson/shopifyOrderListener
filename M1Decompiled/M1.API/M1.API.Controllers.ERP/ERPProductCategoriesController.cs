...
[RoutePrefix("api/ERP/ProductCategories")]
public class ERPProductCategoriesController : ERPBaseController
{
	public IERPProductCategoryModel erpProductCategoryModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPProductCategoryDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPProductCategoryDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllProductCategoryAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpProductCategoryModel = new ERPProductCategoryModel())
		{
			return await RunApiMethod(base.Request, erpProductCategoryModel, () => erpProductCategoryModel.ValidateRequest_GetAllProductCategories(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpProductCategoryModel.Process_GetAllProductCategories(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ProductCategory based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPProductCategoryDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPProductCategoryDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/INVENTORY" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetProductCategoryAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpProductCategoryModel = new ERPProductCategoryModel())
		{
			return await RunApiMethod(base.Request, erpProductCategoryModel, () => erpProductCategoryModel.ValidateRequest_GetProductCategory(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpProductCategoryModel.Process_GetProductCategory(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
