...
[RoutePrefix("api/ERP/ScheduleTrees")]
public class ERPScheduleTreesController : ERPBaseController
{
	public IERPScheduleTreeModel erpScheduleTreeModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPScheduleTreeDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPScheduleTreeDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllScheduleTreeAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpScheduleTreeModel = new ERPScheduleTreeModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTreeModel, () => erpScheduleTreeModel.ValidateRequest_GetAllScheduleTrees(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTreeModel.Process_GetAllScheduleTrees(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ScheduleTree based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPScheduleTreeDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPScheduleTreeDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/SCHEDULING" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetScheduleTreeAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpScheduleTreeModel = new ERPScheduleTreeModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTreeModel, () => erpScheduleTreeModel.ValidateRequest_GetScheduleTree(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTreeModel.Process_GetScheduleTree(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
