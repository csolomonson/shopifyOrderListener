...
[RoutePrefix("api/ERP/ChangeLog")]
public class ERPChangeLogController : ERPBaseController
{
	public IERPChangeLogModel erpChangeLogModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPChangeLogDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPChangeLogDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllChangeLogAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpChangeLogModel = new ERPChangeLogModel())
		{
			return await RunApiMethod(base.Request, erpChangeLogModel, () => erpChangeLogModel.ValidateRequest_GetAllChangeLog(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpChangeLogModel.Process_GetAllChangeLog(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ChangeLog based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPChangeLogDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPChangeLogDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/GENERAL" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetChangeLogAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpChangeLogModel = new ERPChangeLogModel())
		{
			return await RunApiMethod(base.Request, erpChangeLogModel, () => erpChangeLogModel.ValidateRequest_GetChangeLog(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpChangeLogModel.Process_GetChangeLog(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
