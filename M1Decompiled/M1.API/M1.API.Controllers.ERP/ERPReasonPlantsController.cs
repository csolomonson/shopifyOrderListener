...
[RoutePrefix("api/ERP/ReasonPlants")]
public class ERPReasonPlantsController : ERPBaseController
{
	public IERPReasonPlantModel erpReasonPlantModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPReasonPlantDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPReasonPlantDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllReasonPlantAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpReasonPlantModel = new ERPReasonPlantModel())
		{
			return await RunApiMethod(base.Request, erpReasonPlantModel, () => erpReasonPlantModel.ValidateRequest_GetAllReasonPlants(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpReasonPlantModel.Process_GetAllReasonPlants(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ReasonPlant based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPReasonPlantDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPReasonPlantDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/GENERAL" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetReasonPlantAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpReasonPlantModel = new ERPReasonPlantModel())
		{
			return await RunApiMethod(base.Request, erpReasonPlantModel, () => erpReasonPlantModel.ValidateRequest_GetReasonPlant(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpReasonPlantModel.Process_GetReasonPlant(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
