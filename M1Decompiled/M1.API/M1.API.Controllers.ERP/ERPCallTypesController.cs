...
[RoutePrefix("api/ERP/CallTypes")]
public class ERPCallTypesController : ERPBaseController
{
	public IERPCallTypeModel erpCallTypeModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPCallTypeDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPCallTypeDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllCallTypeAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpCallTypeModel = new ERPCallTypeModel())
		{
			return await RunApiMethod(base.Request, erpCallTypeModel, () => erpCallTypeModel.ValidateRequest_GetAllCallTypes(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpCallTypeModel.Process_GetAllCallTypes(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a CallType based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPCallTypeDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPCallTypeDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/CRM" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetCallTypeAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpCallTypeModel = new ERPCallTypeModel())
		{
			return await RunApiMethod(base.Request, erpCallTypeModel, () => erpCallTypeModel.ValidateRequest_GetCallType(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpCallTypeModel.Process_GetCallType(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
