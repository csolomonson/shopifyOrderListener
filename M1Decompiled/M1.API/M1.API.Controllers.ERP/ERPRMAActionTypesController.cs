...
[RoutePrefix("api/ERP/RMAActionTypes")]
public class ERPRMAActionTypesController : ERPBaseController
{
	public IERPRMAActionTypeModel erpRMAActionTypeModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPRMAActionTypeDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPRMAActionTypeDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllRMAActionTypeAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpRMAActionTypeModel = new ERPRMAActionTypeModel())
		{
			return await RunApiMethod(base.Request, erpRMAActionTypeModel, () => erpRMAActionTypeModel.ValidateRequest_GetAllRMAActionTypes(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpRMAActionTypeModel.Process_GetAllRMAActionTypes(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a RMAActionType based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPRMAActionTypeDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPRMAActionTypeDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/QUALITY" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetRMAActionTypeAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpRMAActionTypeModel = new ERPRMAActionTypeModel())
		{
			return await RunApiMethod(base.Request, erpRMAActionTypeModel, () => erpRMAActionTypeModel.ValidateRequest_GetRMAActionType(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpRMAActionTypeModel.Process_GetRMAActionType(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
