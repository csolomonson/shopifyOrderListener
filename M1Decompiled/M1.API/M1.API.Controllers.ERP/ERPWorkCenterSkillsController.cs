...
[RoutePrefix("api/ERP/WorkCenterSkills")]
public class ERPWorkCenterSkillsController : ERPBaseController
{
	public IERPWorkCenterSkillModel erpWorkCenterSkillModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPWorkCenterSkillDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPWorkCenterSkillDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllWorkCenterSkillAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpWorkCenterSkillModel = new ERPWorkCenterSkillModel())
		{
			return await RunApiMethod(base.Request, erpWorkCenterSkillModel, () => erpWorkCenterSkillModel.ValidateRequest_GetAllWorkCenterSkills(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpWorkCenterSkillModel.Process_GetAllWorkCenterSkills(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a WorkCenterSkill based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPWorkCenterSkillDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPWorkCenterSkillDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/EMPLOYEE" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetWorkCenterSkillAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpWorkCenterSkillModel = new ERPWorkCenterSkillModel())
		{
			return await RunApiMethod(base.Request, erpWorkCenterSkillModel, () => erpWorkCenterSkillModel.ValidateRequest_GetWorkCenterSkill(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpWorkCenterSkillModel.Process_GetWorkCenterSkill(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
