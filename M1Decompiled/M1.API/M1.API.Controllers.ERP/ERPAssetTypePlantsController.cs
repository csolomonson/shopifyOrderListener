...
[RoutePrefix("api/ERP/AssetTypePlants")]
public class ERPAssetTypePlantsController : ERPBaseController
{
	public IERPAssetTypePlantModel erpAssetTypePlantModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPAssetTypePlantDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPAssetTypePlantDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllAssetTypePlantAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpAssetTypePlantModel = new ERPAssetTypePlantModel())
		{
			return await RunApiMethod(base.Request, erpAssetTypePlantModel, () => erpAssetTypePlantModel.ValidateRequest_GetAllAssetTypePlants(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpAssetTypePlantModel.Process_GetAllAssetTypePlants(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a AssetTypePlant based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPAssetTypePlantDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPAssetTypePlantDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/FINANCIAL" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetAssetTypePlantAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpAssetTypePlantModel = new ERPAssetTypePlantModel())
		{
			return await RunApiMethod(base.Request, erpAssetTypePlantModel, () => erpAssetTypePlantModel.ValidateRequest_GetAssetTypePlant(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpAssetTypePlantModel.Process_GetAssetTypePlant(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
