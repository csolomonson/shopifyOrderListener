...
[RoutePrefix("api/ERP/SkillCompetencies")]
public class ERPSkillCompetenciesController : ERPBaseController
{
	public IERPSkillCompetencyModel erpSkillCompetencyModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPSkillCompetencyDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPSkillCompetencyDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllSkillCompetencyAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpSkillCompetencyModel = new ERPSkillCompetencyModel())
		{
			return await RunApiMethod(base.Request, erpSkillCompetencyModel, () => erpSkillCompetencyModel.ValidateRequest_GetAllSkillCompetencies(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpSkillCompetencyModel.Process_GetAllSkillCompetencies(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a SkillCompetency based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPSkillCompetencyDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPSkillCompetencyDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/EMPLOYEE" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetSkillCompetencyAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpSkillCompetencyModel = new ERPSkillCompetencyModel())
		{
			return await RunApiMethod(base.Request, erpSkillCompetencyModel, () => erpSkillCompetencyModel.ValidateRequest_GetSkillCompetency(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpSkillCompetencyModel.Process_GetSkillCompetency(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
