...
[RoutePrefix("api/ERP/AssetTypes")]
public class ERPAssetTypesController : ERPBaseController
{
	public IERPAssetTypeModel erpAssetTypeModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPAssetTypeDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPAssetTypeDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllAssetTypeAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpAssetTypeModel = new ERPAssetTypeModel())
		{
			return await RunApiMethod(base.Request, erpAssetTypeModel, () => erpAssetTypeModel.ValidateRequest_GetAllAssetTypes(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpAssetTypeModel.Process_GetAllAssetTypes(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a AssetType based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPAssetTypeDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPAssetTypeDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/FINANCIAL" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetAssetTypeAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpAssetTypeModel = new ERPAssetTypeModel())
		{
			return await RunApiMethod(base.Request, erpAssetTypeModel, () => erpAssetTypeModel.ValidateRequest_GetAssetType(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpAssetTypeModel.Process_GetAssetType(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
