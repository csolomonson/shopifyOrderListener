...
[RoutePrefix("api/ERP/ScheduleBranches")]
public class ERPScheduleBranchesController : ERPBaseController
{
	public IERPScheduleBranchModel erpScheduleBranchModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPScheduleBranchDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPScheduleBranchDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllScheduleBranchAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpScheduleBranchModel = new ERPScheduleBranchModel())
		{
			return await RunApiMethod(base.Request, erpScheduleBranchModel, () => erpScheduleBranchModel.ValidateRequest_GetAllScheduleBranches(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleBranchModel.Process_GetAllScheduleBranches(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ScheduleBranch based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPScheduleBranchDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPScheduleBranchDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/SCHEDULING" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetScheduleBranchAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpScheduleBranchModel = new ERPScheduleBranchModel())
		{
			return await RunApiMethod(base.Request, erpScheduleBranchModel, () => erpScheduleBranchModel.ValidateRequest_GetScheduleBranch(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleBranchModel.Process_GetScheduleBranch(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
