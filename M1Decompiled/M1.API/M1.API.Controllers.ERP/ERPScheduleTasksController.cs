...
[RoutePrefix("api/ERP/ScheduleTasks")]
public class ERPScheduleTasksController : ERPBaseController
{
	public IERPScheduleTaskModel erpScheduleTaskModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPScheduleTaskDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPScheduleTaskDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllScheduleTaskAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpScheduleTaskModel = new ERPScheduleTaskModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTaskModel, () => erpScheduleTaskModel.ValidateRequest_GetAllScheduleTasks(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTaskModel.Process_GetAllScheduleTasks(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ScheduleTask based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPScheduleTaskDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPScheduleTaskDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/SCHEDULING" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetScheduleTaskAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpScheduleTaskModel = new ERPScheduleTaskModel())
		{
			return await RunApiMethod(base.Request, erpScheduleTaskModel, () => erpScheduleTaskModel.ValidateRequest_GetScheduleTask(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleTaskModel.Process_GetScheduleTask(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
