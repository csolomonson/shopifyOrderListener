...
[RoutePrefix("api/ERP/ScheduleAllocations")]
public class ERPScheduleAllocationsController : ERPBaseController
{
	public IERPScheduleAllocationModel erpScheduleAllocationModel;
	...
	/// <returns>A list of <see cref="T:M1.API.DTOs.ERP.ERPScheduleAllocationDto" /> objects</returns>
	/// <remarks>
	...
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<IList<ERPScheduleAllocationDto>>))]
	...
	[AcceptVerbs("GET")]
	[Route("")]
	public async Task<IHttpActionResult> GetAllScheduleAllocationAsync([FromUri] int pageSize = 1000, [FromUri] int pageNumber = 0, [FromUri] string[] filter = null, [FromUri] string orderBy = null)
	{
		using (erpScheduleAllocationModel = new ERPScheduleAllocationModel())
		{
			return await RunApiMethod(base.Request, erpScheduleAllocationModel, () => erpScheduleAllocationModel.ValidateRequest_GetAllScheduleAllocations(pageSize, pageNumber, filter, orderBy).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleAllocationModel.Process_GetAllScheduleAllocations(pageSize, pageNumber, filter, orderBy), showReturnObject: true, showResponseMessage: false, showRecordCount: true);
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// This endpoint retrieves a ScheduleAllocation based on its identifier. If the record does not exist, a 404 Not Found response will be returned.
	/// </remarks>
	/// <param name="key">The Unique Id of the record to be retrieved</param>
	/// <returns>A single <see cref="T:M1.API.DTOs.ERP.ERPScheduleAllocationDto" /> object).</returns>
	[SwaggerResponse(HttpStatusCode.UnsupportedMediaType, Type = typeof(string))]
	[SwaggerResponse(HttpStatusCode.Unauthorized)]
	[SwaggerResponse(HttpStatusCode.OK, Type = typeof(ERPResponseMessageDto<ERPScheduleAllocationDto>))]
	[SwaggerResponse(HttpStatusCode.NotFound, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.BadRequest, Type = typeof(APIResponseMessageDto))]
	[SwaggerResponse(HttpStatusCode.InternalServerError, Type = typeof(APIResponseMessageDto))]
	[SwaggerOperation(null, Tags = new string[] { "ERP/SCHEDULING" })]
	[AcceptVerbs("GET")]
	[Route("{key}")]
	public async Task<IHttpActionResult> GetScheduleAllocationAsync([FromUri(Name = "key")] Guid key)
	{
		using (erpScheduleAllocationModel = new ERPScheduleAllocationModel())
		{
			return await RunApiMethod(base.Request, erpScheduleAllocationModel, () => erpScheduleAllocationModel.ValidateRequest_GetScheduleAllocation(key).ConfigureAwait(continueOnCapturedContext: false).GetAwaiter()
				.GetResult(), () => erpScheduleAllocationModel.Process_GetScheduleAllocation(key), showReturnObject: true, showResponseMessage: false, showRecordCount: false);
		}
	}
}
