...
public class PartQuantityQtyOnOrderSales : PartQuantityDefinition
{
	public PartQuantityQtyOnOrderSales()
	{
		BinQuantityField = "imbQuantityOnOrderSales";
	}
	...
	protected override void UpdateQuantity(M1Database database, SqlTransaction transaction, string partID, string revisionID, string warehouseID, string binID, decimal qtyChange, DataRow sourceRow, DataRowVersion rowVersion)
	{
		...
		SqlCommand sqlCommand = database.NewSqlCommand("UPDATE PartRevisions SET imrQuantityOnOrderSales = imrQuantityOnOrderSales + @QtyChange WHERE imrPartID = @PartID And imrPartRevisionID = @RevisionID");
