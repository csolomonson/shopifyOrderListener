...
public class PartQuantityQtyOnOrderPurchases : PartQuantityDefinition
{
	public PartQuantityQtyOnOrderPurchases()
	{
		BinQuantityField = "imbQuantityOnOrderPurchases";
	}
	...
	protected override void UpdateQuantity(M1Database database, SqlTransaction transaction, string partID, string revisionID, string warehouseID, string binID, decimal qtyChange, DataRow sourceRow, DataRowVersion rowVersion)
	{
		...
		SqlCommand sqlCommand = database.NewSqlCommand("UPDATE PartRevisions SET imrQuantityOnOrderPurchases = imrQuantityOnOrderPurchases + @QtyChange WHERE imrPartID = @PartID And imrPartRevisionID = @RevisionID");
