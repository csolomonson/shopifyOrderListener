...
[SaveAsProcessing("ARInvoiceLines")]
public class ARInvoiceLinesSaveAs : ISaveAsProcessing
{
	public void BeforeUpdate(SaveAsProcessingParms parm)
	{
		if (!parm.Table.Equals("ARInvoiceLines", StringComparison.CurrentCultureIgnoreCase))
		{
			return;
		}
		using SqlCommand sqlCommand = new SqlCommand("SELECT arpARInvoiceID, arpPostedToGL FROM ARInvoices WHERE arpARInvoiceID = @InvID AND arpPostedToGL <> 0");
